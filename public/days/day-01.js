...
window.COURSE_DAY_DATA[1] = {
  ...
  context: `
  <p>The 2026 AI model landscape has four distinct segments. Understanding what lives in each segment — and why — is foundational knowledge for every AI PM.</p>

  <p><strong>Segment 1: Anthropic</strong><br>
  Anthropic’s Claude family is built around Constitutional AI alignment and a tiered capability structure. The current models are <code>claude-opus-4-6</code> (highest capability; now supports a <strong>1M token context window</strong>, making it the go-to for whole-codebase analysis, long document ingestion, and complex multi-step reasoning), <code>claude-sonnet-4-6</code> (the production workhorse; 200K context, best performance-to-cost ratio for most enterprise workloads), and <code>claude-haiku-4-5-20251001</code> (fastest and cheapest; 200K context, ideal for high-volume classification and real-time UX). Always pull the current model strings from <a href="https://docs.anthropic.com/en/docs/about-claude/models" target="_blank">docs.anthropic.com/en/docs/about-claude/models</a> — these change with each release.</p>

  <p><strong>Segment 2: OpenAI</strong><br>
  OpenAI’s lineup splits into two families: the standard (<code>gpt-4o</code>, 128K context, multimodal, strong function-calling) and the reasoning series (<code>o3</code>, <code>o4-mini</code>; chain-of-thought reasoning with extended compute at inference time). The reasoning series excels at multi-step logic, math, and code — at roughly 3–10× the latency and cost of standard models. <code>gpt-4o-mini</code> serves the high-volume economy tier.</p>

  <p><strong>Segment 3: Google</strong><br>
  Gemini 2.5 Pro leads Google’s family with a 1M+ token context window natively — currently the largest context of any production API. Gemini Flash variants offer lower-cost, lower-latency options. Google’s native integration with Search, Workspace, and Vertex AI makes it the default choice for organizations already in the Google ecosystem.</p>
...
monthly cost = daily cost × 30</pre>
  <p>Apply this formula to the code lab below, then cross-check with live pricing pages. Output tokens are typically priced 3–5× higher than input tokens across all providers.</p>`,
